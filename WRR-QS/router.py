			...
			for k in range(sum(numpackets_simplified)):
				...
				for j in range(3):
					...
					try:
						...
						tarr=0
						if len(t_arrive[j])>0:
							tarr = t_arrive[j].pop(0)

						dt += ';'+ ';'.join([str(i) for i in numpackets])+';0;0;'+str(tarr)+'; ;'+str(numpackets_simplified)+';'+str([len(source[0]),len(source[1]),len(source[2])])+';'+str(l_avg_dump)+'; ; ; ; ; ; ; ; ; ; '

						s2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
						s2.connect(daddr)
						print("numpacket now",numpackets)
						print("connect to ", daddr, "and send ", dt)
